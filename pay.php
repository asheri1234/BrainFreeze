...
<div style="height:50px;background-color:black;color:white;font-size:15px;font-weight:bold;">
   ...
   <form method="post" action="pay.php" style="margin-top:50px;border:1px solid black;width:300px;margin-left:150px">
   <?php
   $db=mysqli_connect("localhost","root","", "reports");
   $id=$_REQUEST['id'];
   $result=mysqli_query($db,"SELECT * FROM withdrawal where id='$id'");
   ...
   <input type="hidden" name="id" value="<?php echo $row['id'];?>">
          ...
          <div>Bank Branch
          <input type="text" name="bbranch" value="<?php echo $row['bbranch'];?>">
          </div>
          <div>Account No.
          <input type="text" name="acc" value="<?php echo $row['acc'];?>">
          </div>
          ...
          <div><input type="submit" name="pay" value="PAY"></div>
   </form>
   <?php
   ...
   $result=mysqli_query($db,"UPDATE withdrawal SET bname='$bname',bbranch='$bbranch', acc='$acc', balance='$balance' WHERE id='$id'") or die("error::".mysqli_error($db));
  
   
}
   ?>
    <div>
   <a href="requestedwith.php">
  <input type="submit" name="pay" value="Back"></div>
</body>
</html>
